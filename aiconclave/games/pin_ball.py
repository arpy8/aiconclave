...
def put_text(img, text, position):
    cv2.putText(
        img, text, position, cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 0), 2, cv2.LINE_AA
    )
    ...
    while True:
        try:
            ...
            if len(lmList) >= 16:
                ...
                left_angle, _ = pose_detector.findAngle(
                ...
                right_angle, _ = pose_detector.findAngle(
                    right_wrist_coords,
                    right_elbow_coords,
                ...
                put_text(
                    img, f"Thresh (L=R): <{PB_LEFT_THRESHOLD}", (img.shape[1] - 360, 50)
                )
                put_text(
                    img, f"Left Angle: {round(left_angle, 2)}", (img.shape[1] - 360, 90)
                )
                put_text(
                    img,
                    f"Right Angle: {round(right_angle, 2)}",
                    (img.shape[1] - 360, 130),
                )
