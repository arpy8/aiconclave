...
from aiconclave.constants import (
    TK_FRONT_POINT_COORDS,
    TK_REAR_POINT_COORDS,
    TK_THRESH,
    TK_RPUNCH_ANGLE_LTHRESH,
    TK_RPUNCH_ANGLE_UTHRESH,
    TK_LPUNCH_ANGLE_LTHRESH,
    TK_LPUNCH_ANGLE_UTHRESH,
    TK_RANDOM_UPPER,
    TK_RANDOM_LOWER,
)
...
def launch_game():
    teken_path = pkg_resources.resource_filename(
        __name__, "../assets/tekken/TekkenGame.exe"
    )
    sb.Popen(teken_path)
    return None
...
def put_text(img, text, position):
    cv2.putText(
        img, text, position, cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 2, cv2.LINE_AA
    )
    ...
    while True:
        try:
            ...
            if len(lmList) >= 16:
                ...
                DISTANCE_FROM_FRONT_POINT, _, _ = pose_detector.findDistance(
                    nose_coords,
                    TK_FRONT_POINT_COORDS,
                    img=img,
                    color=(0, 0, 255),
                ...
                DISTANCE_FROM_REAR_POINT, _, _ = pose_detector.findDistance(
                    nose_coords,
                    TK_REAR_POINT_COORDS,
                    img=img,
                    color=(0, 0, 255),
